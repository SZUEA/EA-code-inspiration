...
namespace ModificationTreeDivision
{
    const int MAXN = 1e5 + 10;
    // e是存树的边的
    vector<int> e[MAXN];
...
#define rs(x) ((x << 1) | 1)
    ...
    ll _query_(int u, int l, int r)
    {
        ...
        ll ans = 0;
        if (l <= mid)
            ans += _query_(ls(u), l, mid);
        if (r > mid)
            ans += _query_(rs(u), mid + 1, r);
        return ans;
    }
    // 询问树上距离
    ll query(int x, int y)
    {
        ...
        if (dep[x] < dep[y])
            swap(x, y);
        ans += _query_(1, id[x], id[y]);
        return ans;
    }

    ll queryAll(int x)
    {
        _query_(1, id[x], id[x] + sz[x] - 1);
    }
    ...
    void _update_(int u, int l, int r, int k)
    {
        ...
        ll ans = 0;
        if (l <= mid)
            _update_(ls(u), l, mid, k);
        if (r > mid)
            _update_(rs(u), mid + 1, r, k);
        pushup(u);
    }
